  ...
  describe(filterTestFiles, () => {
    it('should include files under test/', () => {
      const result = filterTestFiles([
        'test/step-definitions/report.js',
        'cucumber.js'
      ])

      expect(result).toStrictEqual(['test/step-definitions/report.js'])
    })

    it('should include files under data/', () => {
      const result = filterTestFiles(['data/data-generator.js', 'README.md'])

      expect(result).toStrictEqual(['data/data-generator.js'])
    })
